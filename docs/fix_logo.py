...
def replace_src(img_tag, new_src):
    img_pattern = re.compile(
        r'<img[^>]*src=[\'"][^\'"]+[\'"][^>]*alt=[\'"]Clickable Image[\'"][^>]*style=[\'"]width:[^\'"]+; min-width:[^\'"]+[\'"][^>]*>'
    )
    modified_img_tag = img_pattern.sub(
        f'<img src="{new_src}" alt="Clickable Image" style="width:200px; min-width:200px">',
        img_tag,
    )
