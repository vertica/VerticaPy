...
class Visualizer:
    ...
    in two boxes, one for the settings and the other for the item to display.
    The item to display can be any thing from a chart to a table.
    ...
    def _accordion(children: list, titles: list) -> widgets.Accordion:
        ...
        Args:
            children (list): list of widgets that will be displayed by the accordions
...
class Item:
    """
    A wrapper class that gets simple widget like IntText, Dropdown, ...,
    and prepend the widget name in a nice button
    """
...
def makeItems(items: list[tuple]) -> dict[str, Item]:
    ...
    Args:
        items (list[tuple]): a list of attributes for Item. Each
        element is a tuple that contains the name, widget and description
        for an Item

    Returns:
        dict[str, Item]: a dict of Item. the key is the name attribute of the Item
    """
    item_dict = {}
    for item in items:
