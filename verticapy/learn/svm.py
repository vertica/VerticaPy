...
test_relation: str
	...
	object
        ...
        query += ", class_weights = '{}'".format(
            ", ".join([str(item) for item in self.class_weight])
        )
    ...
    # ---#
    def predict(self, vdf, name: str = "", cutoff: float = 0.5):
        """
	---------------------------------------------------------------------------
	...
	vDataFrame
        ...
        name = (
            "LinearSVC_" + "".join(ch for ch in self.name if ch.isalnum())
            if not (name)
            else name
        )
...
test_relation: str
	...
	vDataFrame
        ...
        name = (
            "LinearSVR_" + "".join(ch for ch in self.name if ch.isalnum())
            if not (name)
            else name
        )
