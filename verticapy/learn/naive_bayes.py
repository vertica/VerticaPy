...
test_relation: str
	...
	str/list
        ...
        if allSQL:
            sql = "PREDICT_NAIVE_BAYES({} USING PARAMETERS model_name = '{}', class = '{}', type = 'probability', match_by_pos = 'true')".format(
                ", ".join(self.X), self.name, "{}"
            )
            sql = [
                sql,
                "PREDICT_NAIVE_BAYES({} USING PARAMETERS model_name = '{}', match_by_pos = 'true')".format(
                    ", ".join(self.X), self.name
                ),
            ]
        else:
            if pos_label in self.classes and cutoff <= 1 and cutoff >= 0:
                ...
                sql = "PREDICT_NAIVE_BAYES({} USING PARAMETERS model_name = '{}', match_by_pos = 'true')".format(
                    ", ".join(self.X), self.name
                )
        return sql
	...
	vDataFrame
        ...
        if not (name):
            name = "MultinomialNB_" + "".join(ch for ch in self.name if ch.isalnum())
