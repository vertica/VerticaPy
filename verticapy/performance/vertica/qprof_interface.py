...
class QueryProfilerInterface(QueryProfiler):
    """
    A class that inherits QueryProfiler and adds
    ...
    def __init__(
        ...
        self.index_widget = widgets.IntText(description='Index:', value=self.transactions_idx)
        ...
        # graph headers
        self.cpu_header = widgets.HTML(value=f"<h1><b>CPU Time by node and path_id - [transaction_idx: {self.index_widget.value}]</b></h1>")
        self.qpt_header = widgets.HTML(value=f"<h1><b>Query Plan Tree - [transaction_idx: {self.index_widget.value}]</b></h1>")
        self.qsteps_header = widgets.HTML(value=f"<h1><b>Query Execution Steps - [transaction_idx: {self.index_widget.value}]</b></h1>")
    ...
    def get_qplan_tree(self, use_javascript=True):
        ...
        tree_button_box = widgets.HBox([tree_button], layout={'justify_content': 'center'})
        tree_settings = [
        ...
        refresh_pathids_box = widgets.HBox([self.refresh_pathids], layout={'justify_content': 'center'})
        accordion_items = {
            'Metrics': tags,
            'Path ID': widgets.VBox([self.pathid_dropdown.get_item(), refresh_pathids_box]),
            'Tree style': widgets.VBox(tree_settings),
        }
        accordions = Visualizer._accordion(list(accordion_items.values()), accordion_items.keys())
        
        header_box = widgets.HBox([self.qpt_header], layout={'justify_content': 'center'})
        controls = {
            'metric': tags,
        ...
        interactive_output = widgets.interactive_output(self.update_qplan_tree, controls)
        settings = [
            accordions,
    ...
    def update_qplan_tree(
        ...
        if self.use_javascript == False:
            ...
            display(HTML(output))
        self.qpt_header.value = f"<h1><b>Query Plan Tree - [transaction_idx: {index}]</b></h1>"
        
    # Event handlers for the buttons
    def next_button_clicked(self, button):
        """callback function triggered when the user click on the
        button to go to the next transaction/query.
    ...
    def prev_button_clicked(self, button):
        """callback function triggered when the user click on the
        button to go to the previous transaction/query
...
##########################################################################        
    ...
    def get_qsteps(self):
        ...
        self.qsteps_items = makeItems([
            ...
            (
                "categoryorder",
                widgets.Dropdown(options=QprofUtility._get_categoryorder(), value="sum descending"),
        ...
        qsteps_button_box = widgets.HBox([qsteps_button], layout={'justify_content': 'center'})
        ...
        header_box = widgets.HBox([self.qsteps_header], layout={'justify_content': 'center'})
        settings_layout = {}
        graph_layout = {}
...
#############################################################
    ...
    def get_cpu_time(self):
        ...
        cpu_items = makeItems([
            ...
            (
                "categoryorder",
                widgets.Dropdown(options=QprofUtility._get_categoryorder(), value="max descending"),
                "How to sort the bars"
            )
        ])
        ...
        header_box = widgets.HBox([self.cpu_header], layout={'justify_content': 'center'})
        cpu_settings = [
...
##########################################################
    ...
    def step(self):
        ...
        interactive_output = widgets.interactive_output(self.update_step, {'step_idx': self.step_idx})
        out = widgets.Output()
        with out:
    ...
    def update_step(self, step_idx):
        """callback function that calls a step function
