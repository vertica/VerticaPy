...
----------
	...
	vDataFrame.eval     : Evaluates a customized expression.
		...
		try:
			try:
				ctype = get_data_types("SELECT {} AS apply_test_feature FROM {} WHERE {} IS NOT NULL LIMIT 0".format(func.replace("{}", self.alias), self.parent.__genSQL__(), self.alias), self.parent._VERTICAPY_VARIABLES_["cursor"], "apply_test_feature", self.parent._VERTICAPY_VARIABLES_["schema_writing"])
			except:
				ctype = get_data_types("SELECT {} AS apply_test_feature FROM {} WHERE {} IS NOT NULL LIMIT 0".format(func.replace("{}", self.alias), self.parent.__genSQL__(), self.alias), self.parent._VERTICAPY_VARIABLES_["cursor"], "apply_test_feature")
	...
	vDataFrame.aggregate : Computes the vDataFrame input aggregations.
		...
		if ((is_date) and not(method == "categorical")):
			...
			query_on, time_on, title = self.parent._VERTICAPY_VARIABLES_["query_on"], self.parent._VERTICAPY_VARIABLES_["time_on"], "Describes the statics of {} partitioned by {}.".format(numcol, self.alias)
			result = to_tablesample(query, self.parent._VERTICAPY_VARIABLES_["cursor"], query_on = query_on, time_on = time_on, title = title)
	...
	vDataFrame[].mean_encode  : Encodes the vcolumn using the Mean Encoding of a response.
		...
		if (self.isnum() and method == "smart"):
			...
			while (total < count - 1):
				nth_elems += [str(total)]
				total += nb
			where = "WHERE _verticapy_row_nb_ IN ({})".format(", ".join(['1'] + nth_elems + [str(count)]))
	...
	vDataFrame.drop: Drops the input vcolumns from the vDataFrame.
		...
		try:
			parent = self.parent
			force_columns = [column for column in self.parent._VERTICAPY_VARIABLES_["columns"]]
			force_columns.remove(self.alias)
			self.parent._VERTICAPY_VARIABLES_["cursor"].execute("SELECT * FROM {} LIMIT 10".format(self.parent.__genSQL__(force_columns = force_columns)))
	...
	vDataFrame.outliers      : Adds a new vcolumn labeled with 0 and 1 
		...
		if (method not in ("winsorize", "null", "mean")):
			...
			if (use_threshold):
				...
				p_alpha, p_1_alpha = self.parent._VERTICAPY_VARIABLES_["cursor"].fetchone()
			if (method == "winsorize"):
				...
				mean_alpha, mean_1_alpha = [item[0] for item in self.parent._VERTICAPY_VARIABLES_["cursor"].fetchall()]
		...
		if (val != None):
			...
			if (by == []):
				...
				try:
					...
					self.parent._VERTICAPY_VARIABLES_["cursor"].execute("SELECT {} FROM {} LIMIT 1".format(new_column.format(self.alias), self.parent.__genSQL__()))
				except:
	...
	vDataFrame.aggregate : Computes the vDataFrame input aggregations.
		...
		self.parent.__executeSQL__("SELECT {} FROM (SELECT {}, COUNT(*) AS _verticapy_cnt_ FROM {}{}GROUP BY {} ORDER BY _verticapy_cnt_ DESC LIMIT {}) x ORDER BY _verticapy_cnt_ ASC LIMIT 1".format(self.alias, self.alias, self.parent.__genSQL__(), where, self.alias, n))
		try:
	...
	vDataFrame[].nsmallest : Returns the n nsmallest vcolumn elements.
		...
		query_on, time_on, title = self.parent._VERTICAPY_VARIABLES_["query_on"], self.parent._VERTICAPY_VARIABLES_["time_on"], "Reads {} {} largest elements.".format(self.alias, n)
		return (to_tablesample(query, self.parent._VERTICAPY_VARIABLES_["cursor"], name = "nlargest", query_on = query_on, time_on = time_on, title = title))
	#---#
	def normalize(self, 
				  method: str = "zscore", 
				  by: list = [], 
			...
			if (method == "zscore"):
				if (n == 0):
					...
					try:
						...
						self.parent._VERTICAPY_VARIABLES_["cursor"].execute("SELECT {}, {} FROM {} LIMIT 1".format(avg, stddev, self.parent.__genSQL__()))
					except:
				...
				if (n == 0):
					...
					try:
						...
						self.parent._VERTICAPY_VARIABLES_["cursor"].execute("SELECT {}, {} FROM {} LIMIT 1".format(cmax, cmin, self.parent.__genSQL__()))
					except:
	...
	vDataFrame[].nlargest : Returns the n largest vcolumn elements.
		...
		query_on, time_on, title = self.parent._VERTICAPY_VARIABLES_["query_on"], self.parent._VERTICAPY_VARIABLES_["time_on"], "Reads {} {} smallest elements.".format(n, self.alias)
		return (to_tablesample(query, self.parent._VERTICAPY_VARIABLES_["cursor"], name = "nsmallest", query_on = query_on, time_on = time_on, title = title))
	#---#
	def numh(self, 
			 method: str = "auto"):
		"""
	...
	Parameters
 	...
 	float
		...
		if (self.isnum()):
			...
			min_date = self.min()
			table = "(SELECT DATEDIFF('second', '{}'::timestamp, {}) AS {} FROM {}) VERTICAPY_OPTIMAL_H_TABLE".format(min_date, self.alias, self.alias, self.parent.__genSQL__())
	...
	vDataFrame[].head : Returns the vcolumn head.
		...
		query_on, time_on, title = self.parent._VERTICAPY_VARIABLES_["query_on"], self.parent._VERTICAPY_VARIABLES_["time_on"], "Reads {}.".format(self.alias)
		tail = to_tablesample("SELECT {} AS {} FROM {} LIMIT {} OFFSET {}".format(convert_special_type(self.category(), False, self.alias), self.alias, self.parent.__genSQL__(), limit, offset), self.parent._VERTICAPY_VARIABLES_["cursor"], query_on = query_on, time_on = time_on, title = title)
	...
	vDataFrame[].describe : Computes the vcolumn descriptive statistics.
		...
		try:
			...
			query = "SELECT {} AS {}, COUNT(*) AS _verticapy_cnt_, 100 * COUNT(*) / {} AS percent FROM {} GROUP BY {} ORDER BY _verticapy_cnt_ DESC {}".format(convert_special_type(self.category(), True, self.alias), self.alias, self.parent.shape()[0], self.parent.__genSQL__(), self.alias, topk)
			self.parent.__executeSQL__(query, title = "Computes the top{} categories of {}.".format(k if k > 0 else "", self.alias))
		result = self.parent._VERTICAPY_VARIABLES_["cursor"].fetchall()
