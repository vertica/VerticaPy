...
from verticapy.connection.errors import OAuthTokenRefreshError
...
class OAuthManager:
    ...
    def set_config(self, configs) -> None:
        valid_keys = {'refresh_token', 'client_id', 'client_secret', 'token_url', 'discovery_url',
                      'scope', 'tls_check_hostname', 'tls_verify'}
        try:
    ...
    def get_access_token_using_refresh_token(self):
        ...
        try:
            ...
            response = s.post(self.token_url, headers=headers, data=params, verify=self.tls_verify)
    ...
    def _get_token_url_from_discovery_url(self) -> str:
        try:
            ...
            if not self.tls_check_hostname:
                s.mount('https://', HostNameIgnoringAdapter())
            response = s.get(self.discovery_url, headers=headers, verify=self.tls_verify)
            response.raise_for_status()
