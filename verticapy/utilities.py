...
pandas_to_vertica : Ingests a pandas DataFrame into the Vertica database.
    ...
    if copy and not (genSQL):
        ...
        executeSQL(
            sql,
            title="Insert new lines in the {} table. The batch insert is converted into a COPY statement by using prepared statements.".format(
                table_name
            ),
...
def set_option(option: str, value: Union[bool, int, str] = None):
    ...
    if option == "cache":
        ...
        if isinstance(value, str):
            query = "SELECT table_schema FROM columns WHERE table_schema = '{}' LIMIT 1;".format(
                value.replace("'", "''")
            )
            res = executeSQL(
            ...
            if res:
                ...
                raise ParameterError(
                    "The schema '{}' could not be found.".format(value)
                )
    elif option == "time_on":
        check_types([("value", value, [bool])])
        if isinstance(value, bool):
