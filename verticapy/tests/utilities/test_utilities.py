...
class TestUtilities:
    def test_create_verticapy_schema(self, base):
        ...
        base.cursor.execute("SELECT table_name FROM columns WHERE table_schema = 'verticapy' GROUP BY 1 ORDER BY 1;")
    ...
    def test_drop(self, base, world_vd):
        ...
        base.cursor.execute("SELECT table_name FROM columns WHERE table_name = 'verticapy_table_test' GROUP BY 1;")
        ...
        base.cursor.execute("SELECT table_name FROM view_columns WHERE table_name = 'verticapy_view_test' GROUP BY 1;")
        result = base.cursor.fetchall()
        assert result == []
        # text index
        base.cursor.execute("CREATE TEXT INDEX drop_index ON drop_data (id, transportation);")
        ...
        base.cursor.execute("SELECT NAIVE_BAYES('public.verticapy_model_test', 'public.drop_data', 'transportation', 'gender, cost');")
        drop("verticapy_model_test", base.cursor)
        base.cursor.execute("SELECT model_name FROM models WHERE model_name = 'verticapy_model_test' GROUP BY 1;")
        result = base.cursor.fetchall()
        assert result == []
        # verticapy model
        with warnings.catch_warnings(record=True) as w:
        ...
        drop("verticapy_model_test", base.cursor)
        base.cursor.execute("SELECT model_name FROM verticapy.models WHERE model_name = 'verticapy_model_test' GROUP BY 1;")
        result = base.cursor.fetchall()
        assert result == []
        drop("verticapy", base.cursor, method = "schema")
        # geo index
        ...
        drop("world_polygons", base.cursor,)
        try:
    ...
    def test_pandas_to_vertica(self, titanic_vd):
        ...
        with warnings.catch_warnings(record=True) as w:
            drop("titanic_pandas", titanic_vd._VERTICAPY_VARIABLES_["cursor"],)
        pandas_to_vertica(df = df, 
                          cursor = titanic_vd._VERTICAPY_VARIABLES_["cursor"], 
                          name = "titanic_pandas")
        vdf = vDataFrame("titanic_pandas", cursor = titanic_vd._VERTICAPY_VARIABLES_["cursor"])
    ...
    def test_pjson(self, base):
        result = pjson(os.path.dirname(verticapy.__file__) + "/tests/utilities/titanic-passengers.json", 
                      base.cursor)
        assert result == {'datasetid': 'Varchar(36)',
                          'fields.age': 'Float',
    ...
    def test_read_json(self, base):
        with warnings.catch_warnings(record=True) as w:
            drop("public.titanic_verticapy_test", base.cursor,)
        result = read_json(os.path.dirname(verticapy.__file__) + "/tests/utilities/titanic-passengers.json", 
                           base.cursor,
    ...
    def test_read_csv(self, base):
        with warnings.catch_warnings(record=True) as w:
            drop("public.titanic_verticapy_test", base.cursor,)
        result = read_csv(os.path.dirname(verticapy.__file__) + "/data/titanic.csv", 
                          base.cursor,
                          table_name = "titanic_verticapy_test")
    ...
    def test_tablesample(self, base):
        result = tablesample({"index": ["Apple", "Banana", "Orange"],
                              "price": [1, 2, 3]})
        ...
        assert result6 == '(SELECT \'Apple\' AS "index", 1 AS "price") UNION ALL (SELECT \'Banana\' AS "index", 2 AS "price") UNION ALL (SELECT \'Orange\' AS "index", 3 AS "price")'
    ...
    def test_vdf_from_relation(self, base):
        result = vdf_from_relation('(SELECT 1 AS "verticapy test *+") x', cursor = base.cursor,)
