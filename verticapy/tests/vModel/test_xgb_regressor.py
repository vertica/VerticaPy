...
class TestXGBR:
    ...
    def test_set_cursor(self, base):
        model_test = XGBoostRegressor("xgbr_cursor_test", cursor=base.cursor)
        # TODO: creat a new cursor
        model_test.set_cursor(base.cursor)
        model_test.drop()
        model_test.fit("public.xgbr_data", ["gender"], "transportation")

        base.cursor.execute(
            "SELECT model_name FROM models WHERE model_name = 'xgbr_cursor_test'"
        )
        assert base.cursor.fetchone()[0] == "xgbr_cursor_test"
