...
class TestXGBC:
    ...
    def test_to_python(self, model, titanic_vd):
        ...
        model_test.cursor.execute(
            "SELECT PREDICT_XGB_CLASSIFIER(* USING PARAMETERS model_name = 'rfc_python_test', match_by_pos=True) FROM (SELECT 30.0 AS age, 45.0 AS fare, 'male' AS sex) x"
        )
        prediction = model_test.cursor.fetchone()[0]
        assert prediction == model_test.to_python(return_str=False)([[30.0, 45.0, 'male']])[0]
        model_test.cursor.execute(
            "SELECT PREDICT_XGB_CLASSIFIER(* USING PARAMETERS model_name = 'rfc_python_test', match_by_pos=True) FROM (SELECT 30.0 AS age, 145.0 AS fare, 'female' AS sex) x"
        )
        prediction = model_test.cursor.fetchone()[0]
