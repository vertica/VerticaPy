...
def vpy_lr_model_pred(winequality_vpy, load_test_schema):
    vpy_lr_model = vpy_linear_model.LinearRegression(
        f"{load_test_schema}.vpy_lr_model",
    )
    vpy_lr_model.drop()
    vpy_lr_model.fit(
        f"{load_test_schema}.winequality",
    ...
    vpy_lr_model.drop()
    drop(
        name=f"{load_test_schema}.vpy_lr_pred",
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
                ...
                {},
                "n",
        ),  # need to check with Badr on displaying message on using mse for aic. statsmodel uses
        # OLS and gives different result
        (
                "bic_score",
                "bic",
                {},
                "n",
        ),  # need to check with Badr on displaying message on using mse for bic. statsmodel
        # uses OLS and gives different result
        (
                "explained_variance",
        ...
        (
                ...
                {},
                "n",
        ),  # need to check with Badr on displaying message on log base, vertica has default base
        # 10, sklean uses natural log (e)
        (
...
class TestRegressionMetrics:
    def test_master_regression_metrics(
        ...
        vpy_lr_pred_vdf.to_db(
            name=f"{load_test_schema}.vpy_lr_pred", relation_type=f"{sql_relation_type}"
        )
        if vpy_regression_metrics in ["quantile_error"]:
            vpy_res = getattr(vpy_metrics, vpy_regression_metrics)(
                y_true="quality",
        ...
        if is_skl_metrics == "y":
            if py_regression_metrics in ["r2_score"]:
                ...
                if func_params["adj"]:
                    skl_res = 1 - (1 - r2_score) * (len(vpy_lr_pred_pdf) - 1) / (
                            len(vpy_lr_pred_pdf) - num_pred - 1
                    )
                else:
                    skl_res = r2_score
            else:
                skl_res = getattr(skl_metrics, py_regression_metrics)(
                    vpy_lr_pred_pdf["quality"], vpy_lr_pred_pdf["quality_pred"]
                )
        else:
