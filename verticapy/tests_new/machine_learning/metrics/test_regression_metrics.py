...
@pytest.mark.parametrize(
    ...
    [
        ("aic_score", "aic", {}, "n",),  # need to check with Badr on displaying message/in doc on using mse for aic. statsmodel uses OLS(ordnary least sqauare) as default and gives different result
        ("bic_score", "bic", {}, "n", ),  # need to check with Badr on displaying message/in doc on using mse for bic. statsmodel uses OLS(ordnary least sqauare) as default and gives different result
        ...
        def get_vertica_metrics():
            ...
            if vpy_regression_metrics in ["quantile_error"]:
                _vpy_res = getattr(vpy_metrics, vpy_regression_metrics)(
                    y_true="quality", y_score="quality_pred", input_relation=f"{load_test_schema}.vpy_lr_pred",
                    q=0.72)
        ...
        def get_python_metrics():
            if is_skl_metrics == "y":
                if py_regression_metrics in ["r2_score"]:
                    ...
                    _skl_res = getattr(skl_metrics, py_regression_metrics)(vpy_lr_pred_pdf["quality"],
                                                                           vpy_lr_pred_pdf["quality_pred"])
            else:
                if py_regression_metrics in ["quantile_error"]:
                    _skl_res = abs(vpy_lr_pred_pdf["quality"] - vpy_lr_pred_pdf["quality_pred"]).quantile(0.72)
                    ...
                    _skl_res = sum(
                        pow((np.log10(vpy_lr_pred_pdf["quality_pred"] + 1) -
                             np.log10(vpy_lr_pred_pdf["quality"] + 1)), 2)
                    ) / len(vpy_lr_pred_pdf)
                else:
