...
#
# ---#
def import_lib_udf(udf_list: list,
				   library_name: str,
...
cursor: DBcursor, optional
	...
	try:
		...
		pass
	udx_str, sql = create_lib_udf(udf_list, library_name, include_dependencies, directory + "/" + file_name)
...
# ---#
def create_lib_udf(udf_list: list,
				   library_name: str,
				   include_dependencies: list = [],
				   file_path: str = "",
...
udx_str, sql
	...
	if not(isinstance(include_dependencies, (list))):
		raise ValueError(f"The parameter include_dependencies type must be <list>. Found {type(include_dependencies)}.")
	if not(isinstance(library_name, str)):
		raise ValueError(f"The parameter library_name type must be <str>. Found {type(library_name)}.")
	if not(isinstance(file_path, str)):
		raise ValueError(f"The parameter file_path type must be <str>. Found {type(file_path)}.")
	if not(isinstance(create_file, bool)):
		raise ValueError(f"The parameter create_file type must be <bool>. Found {type(create_file)}.")
	...
	if "__main__" in all_libraries: all_libraries.remove("__main__")
	...
	for udf in udf_list:
		...
		udx_str += tmp[0]
		sql += [tmp[1]]
	sql_path = os.path.dirname(file_path) + "/" + library_name + ".sql" if (file_path) else library_name + ".sql"
	if not(file_path):
		file_path = f"verticapy_{library_name}.py"
	sql = [f"CREATE OR REPLACE LIBRARY {library_name} AS '{file_path}' LANGUAGE 'Python';"] + sql
	if create_file:
		f = open(file_path, "w")
		f.write(udx_str)
		f.close()
...
def create_udf(function,
	...
	if not(hasattr(function, '__call__')):
		raise ValueError(f"The function parameter must be a Python function. Found {type(function)}.")
	if not(new_name):
		new_name = function.__name__
	elif not(isinstance(new_name, str)):
		raise ValueError(f"The parameter new_name type must be <str>. Found {type(new_name)}.")
	module = function.__module__
	if module == "__main__": module = ""
	if not(library_name):
		library_name = module
	if (module):
		module += "."
	...
	if isinstance(arg_types, dict):
		...
		raise ValueError(f"The arg_types parameter must be a <list> of <types>. Found {type(arg_types)}.")
	for idx, dtype in enumerate(arg_types):
		if not(isinstance(dtype, type)):
			raise ValueError(f"Each element of arg_types parameter must be a <type>. Found {type(dtype)} at index {idx}.")
	if isinstance(return_type, dict):
		if len(return_type) == 0:
			raise ValueError("return_type is empty. The returned type must have at least one element.")
	...
	if isinstance(return_type, dict):
		...
		raise ValueError(f"return_type must be the dictionary of the returned types in case of Transform Function and only a type in case of Scalar Function. Can not be of type {type(return_type)}.")
	...
	udx_str += f"\tdef {process_function}(self, server_interface, arg_reader, res_writer):\n"
	udx_str += "\t\twhile(True):\n"
	udx_str += "\t\t\tinputs  = []\n"
	...
	udx_str += f"\t\t\tresult = {module}{function.__qualname__}(*inputs, **self.params)\n"
	if is_udtf:
