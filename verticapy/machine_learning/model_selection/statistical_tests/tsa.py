...
def adfuller(
    ...
    if with_trend:
        predictors += ["ts"]
    try:
        model.fit(relation_name, predictors, "delta", return_report=True,)
    except QueryError:
...
def cochrane_orcutt(
    ...
    model_tmp.drop()
    model_tmp.fit(vdf, X, y, return_report=True,)
    model_tmp.pho_ = pho
...
def seasonal_decompose(
    ...
    if polynomial_order <= 0:
        ...
        model.drop()
        model.fit(vdf_poly, X, column, return_report=True,)
    ...
    if estimate_seasonality:
        ...
        model = LinearRegression(name=name, solver="bfgs", max_iter=100, tol=1e-6)
        model.drop()
        model.fit(vdf_seasonality, X, seasonal_name, return_report=True,)
        vdf[
            seasonal_name
        ] = f"""
