...
pos_label: int/float/str, optional
	Label used to identify the positive class. If pos_label is NULL then the
...
tablesample
	...
	values = {"index": ["auc", "prc_auc", "accuracy", "log_loss", "precision", "recall", "f1_score", "mcc", "informedness", "markedness", "csi", "cutoff"]}
	for idx, elem in enumerate(labels):
		...
		try:
			...
			try:
				tn, fp, fn, tp = matrix.values[0][0], matrix.values[0][1], matrix.values[1][0], matrix.values[1][1]
			except:
				tn, fp, fn, tp = matrix.values["0"][0], matrix.values["0"][1], matrix.values["1"][0], matrix.values["1"][1]
		...
		if (estimator):
			...
			if not(cutoff):
				current_cutoff = roc_curve(y_t, y_p, input_relation, cursor, best_threshold = True)
			elif (type(cutoff) == list):
				if (len(cutoff) == 1):
					current_cutoff = cutoff[0]
				else:
		...
		values[elem] = [auc_score, prc_auc_score, accuracy, logloss, ppv, tpr, f1, mcc, bm, mk, csi, current_cutoff]
	if not(estimator):
		if (conn):
			conn.close()
	return (tablesample(values, table_info = False))
