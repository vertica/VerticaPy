...
----------
	...
	def  __init__(self, 
				  ...
				  parent = None,
				  catalog: dict = {}):
		self.parent, self.alias, self.transformations = parent, alias, [elem for elem in transformations]
		self.catalog = {}
		for method in ["cov", "pearson", "spearman", "kendall", "cramer", "biserial"]: 
			self.catalog[method] = {}
		for method in ["regr_avgx", "regr_avgy", "regr_count", "regr_intercept", "regr_r2", "regr_slope", "regr_sxx", "regr_sxy", "regr_syy"]: 
			self.catalog[method] = {}
		for elem in catalog:
	...
	def __executeSQL__(self, 
		...
		It is used to simplify the execution of some methods.
		"""
	...
	vDataFrame.eval : Evaluates a customized expression.
		...
		self.parent._VERTICA_ML_PYTHON_VARIABLES_["columns"] += [name]
	...
	Parameters
 	----------
 	func: str,
 		Function used to transform the vcolumn. It must be pure SQL.
	...
	vDataFrame.eval     : Evaluates a customized expression.
		...
		try:
			try:
				ctype = get_data_types("SELECT {} AS apply_test_feature FROM {} WHERE {} IS NOT NULL LIMIT 10".format(func.replace("{}", self.alias), self.parent.__genSQL__(), self.alias), self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"], "apply_test_feature", self.parent._VERTICA_ML_PYTHON_VARIABLES_["schema_writing"])
			except:
				ctype = get_data_types("SELECT {} AS apply_test_feature FROM {} WHERE {} IS NOT NULL LIMIT 10".format(func.replace("{}", self.alias), self.parent.__genSQL__(), self.alias), self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"], "apply_test_feature")
			category = category_from_type(ctype = ctype)
			all_cols, max_floor = self.parent.get_columns(), 0
			for column in all_cols:
				if ((str_column(column) in func) or (re.search(re.compile('\\b{}\\b'.format(column.replace('"', ''))), func))):
			...
			if (copy):
				...
				for k in range(max_floor):
					self.parent[copy_name].transformations += [("{}", self.ctype(), self.category())]
				self.parent[copy_name].transformations += [(func, ctype, category)]
				self.parent[copy_name].catalog = self.catalog
				self.parent.__add_to_history__("[{}]: The vcolumn '{}' was transformed with the func 'x -> {}'.".format(func.replace("{}", ""), copy_name.replace('"', ''), func.replace("{}", "x")))
			else:
				for k in range(max_floor):
				...
				self.parent.__add_to_history__("[{}]: The vcolumn '{}' was transformed with the func 'x -> {}'.".format(func.replace("{}", ""), self.alias.replace('"', ''), func.replace("{}", "x")))
			return (self.parent)
	...
	Parameters
 	----------
 	func: str
 		Function used to transform the vcolumn.
			abs     : absolute value
	...
	vDataFrame[].apply : Applies a function to the vcolumn.
		"""
		check_types([
			("func", func, ["abs", "acos", "asin", "atan", "cbrt", "ceil", "cos", "cosh", "cot", "exp", "floor", "ln", "log", "log10", "mod", "pow", "round", "sign", "sin", "sinh", "sqrt", "tan", "tanh"], True),
			("x", x, [int, float], False)])
	...
	vDataFrame.astype : Converts the vcolumns to the input type.
		...
		try:
			query = "SELECT {}::{} AS {} FROM {} WHERE {} IS NOT NULL LIMIT 20".format(self.alias, dtype, self.alias, self.parent.__genSQL__(), self.alias)
			self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].execute(query)
			self.transformations += [("{}::{}".format('{}', dtype), dtype, category_from_type(ctype = dtype))]
			self.parent.__add_to_history__("[AsType]: The vcolumn {} was converted to {}.".format(self.alias, dtype))
			return (self.parent)
		except Exception as e:
			raise Exception("{}\nThe vcolumn {} can not be converted to {}".format(e, self.alias, dtype))
	#---#
	def avg(self):
		"""
	---------------------------------------------------------------------------
	...
	Parameters
 	----------
 	by: str, optional
 		vcolumn used to partition the data.
 	h: float, optional
	...
	vDataFrame[].mean_encode  : Encodes the vcolumn using the Mean Encoding of a response.
		...
		if (type(others) == str): others = "'{}'".format(others.replace("'", "''"))
		if (others == None): others = "NULL"
	...
	vDataFrame[].hist : Draws the Histogram of the vcolumn based on an aggregation.
		"""
		check_types([
			("kernel", kernel, ["gaussian", "logistic", "sigmoid", "silverman"], True), 
	...
	Parameters
 	...
 	max_cardinality: int, optional
 		Cardinality threshold used to determine if the vcolumn will be considered
 		as categorical.
 	numcol: str, optional
	...
	vDataFrame.aggregate : Computes the vDataFrame input aggregations.
		"""
		check_types([
			("method", method, ["auto", "numerical", "categorical", "cat_stats"], True), 
			("max_cardinality", max_cardinality, [int, float], False), 
		...
		if ((is_date) and not(method == "categorical")):
			...
			query = "WITH vdf_table AS (SELECT * FROM {}) {}".format(self.parent.__genSQL__(), " UNION ALL ".join(query))
			query_on, time_on, title = self.parent._VERTICA_ML_PYTHON_VARIABLES_["query_on"], self.parent._VERTICA_ML_PYTHON_VARIABLES_["time_on"], "Describes the statics of {} partitioned by {}.".format(numcol, self.alias)
			result = to_tablesample(query, self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"], query_on = query_on, time_on = time_on, title = title)
			...
			query = "WITH vdf_table AS (SELECT * FROM {}) {}".format(self.parent.__genSQL__(), query)
			self.parent.__executeSQL__(query = query, title = "Computes the descriptive statistics of {}.".format(self.alias))
			query_result = self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].fetchall()
			result = [distinct_count] + [item[1] for item in query_result]
			index = ['unique'] + [item[0] for item in query_result]
		else:
			result = self.parent.describe(method = "numerical", columns = [self.alias], unique = False).transpose().values[self.alias.replace('"', '')]
	...
	vDataFrame[].mean_encode  : Encodes the vcolumn using the Mean Encoding of a response.
		...
		check_types([
			...
			("bins", bins, [int, float], False), 
			("method", method, ["auto", "smart", "same_width", "same_freq", "topk"], True), 
			("return_enum_trans", return_enum_trans, [bool], False)])
		method = method.lower()
		if (self.isnum() and method == "smart"):
			schema = self.parent._VERTICA_ML_PYTHON_VARIABLES_["schema_writing"]
			if not(schema): schema = "public"
			rand_int = random.randint(0, 10000000)
			temp_information = ("{}.VERTICA_ML_PYTHON_TEMP_VIEW_{}".format(schema, rand_int), "{}.VERTICA_ML_PYTHON_TEMP_MODEL_{}".format(schema, rand_int))
			if (bins < 2):
			...
			try:
				self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].execute("DROP VIEW IF EXISTS {}".format(temp_information[0]))
			except:
				try:
					self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].execute("DROP MODEL IF EXISTS {}".format(temp_information[1]))
				except:
					pass
			self.parent.to_db(temp_information[0])
			from vertica_ml_python.learn.ensemble import RandomForestClassifier
			model = RandomForestClassifier(temp_information[1], self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"], n_estimators = 20, max_depth = 3, nbins = 100, min_samples_leaf = min_bin_size)
			...
			result = self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].fetchall()
			...
			while (total < count - 1):
				nth_elems += [str(total)]
				total += nb
			where = "WHERE _vertica_ml_python_row_nb_ IN ({})".format(", ".join(['1'] + nth_elems + [str(count)]))
			query = "SELECT {} FROM (SELECT {}, ROW_NUMBER() OVER (ORDER BY {}) AS _vertica_ml_python_row_nb_ FROM {} WHERE {} IS NOT NULL) x {}".format(
				self.alias, self.alias, self.alias, self.parent.__genSQL__(), self.alias, where)
			self.parent.__executeSQL__(query = query, title = "Computes the equal frequency histogram bins.")
			result = self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].fetchall()
			result = [elem[0] for elem in result]
			...
			if (h <= 0):
				...
				if (h > 0.01):
					...
					h = round(h, 6)
				if (self.category() == "int"): h = int(max(math.floor(h), 1)) 
		...
		if (return_enum_trans):
			...
			try:
				if "count" in sauv:
					...
					self.catalog["percent"] = 100 * sauv["count"] / self.parent.shape()[0]
			except:
				pass
			self.parent.__add_to_history__("[Discretize]: The vcolumn {} was discretized.".format(self.alias))
		return (self.parent)
	#---#
	def distinct(self):
		"""
	...
	vDataFrame.topk : Returns the vcolumn most occurent elements.
		...
		query = "SELECT {} AS {} FROM {} WHERE {} IS NOT NULL GROUP BY {} ORDER BY {}".format(
			convert_special_type(self.category(), True, self.alias), self.alias, self.parent.__genSQL__(), self.alias, self.alias, self.alias)
		self.parent.__executeSQL__(query = query, title = "Computes the distinct categories of {}.".format(self.alias))
		query_result = self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].fetchall()
	...
	vDataFrame.drop: Drops the input vcolumns from the vDataFrame.
		...
		try:
			parent = self.parent
			force_columns = [column for column in self.parent._VERTICA_ML_PYTHON_VARIABLES_["columns"]]
			force_columns.remove(self.alias)
			self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].execute("SELECT * FROM {} LIMIT 10".format(self.parent.__genSQL__(force_columns = force_columns)))
			self.parent._VERTICA_ML_PYTHON_VARIABLES_["columns"].remove(self.alias)
			delattr(self.parent, self.alias)
		except:
			self.parent._VERTICA_ML_PYTHON_VARIABLES_["exclude_columns"] += [self.alias]
		if (add_history):
			self.parent.__add_to_history__("[Drop]: vcolumn {} was deleted from the vDataFrame.".format(self.alias))
		return (parent)
	#---#
	...
	vDataFrame.outliers      : Adds a new vcolumn labeled with 0 and 1 
		...
		if (use_threshold):
			...
			p_alpha, p_1_alpha = self.parent.quantile([alpha, 1 - alpha], [self.alias]).transpose().values[self.alias]
	...
	Parameters
		----------
		method: str, optional
			Method used to fill the vcolumn outliers.
	...
	vDataFrame.outliers      : Adds a new vcolumn labeled with 0 and 1 
		(1 meaning global outlier).
		"""
		check_types([("method", method, ["winsorize", "null", "mean"], True), 
					 ("alpha", alpha, [int, float], False), 
					 ("use_threshold", use_threshold, [bool], False), 
					 ("threshold", threshold, [int, float], False)])
		method = method.lower()
		if (method not in ("winsorize", "null", "mean")):
			...
			if (use_threshold):
				...
				query = "SELECT PERCENTILE_CONT({}) WITHIN GROUP (ORDER BY {}) OVER (), PERCENTILE_CONT(1 - {}) WITHIN GROUP (ORDER BY {}) OVER () FROM {} LIMIT 1".format(alpha, self.alias, alpha, self.alias, self.parent.__genSQL__())
				self.parent.__executeSQL__(query = query, title = "Computes the quantiles of {}.".format(self.alias))
				p_alpha, p_1_alpha = self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].fetchone()
			if (method == "winsorize"):
				...
				query = "WITH vdf_table AS (SELECT * FROM {}) (SELECT AVG({}) FROM vdf_table WHERE {} < {}) UNION ALL (SELECT AVG({}) FROM vdf_table WHERE {} > {})".format(
					self.parent.__genSQL__(), self.alias, self.alias, p_alpha, self.alias, self.alias, p_1_alpha)
				self.parent.__executeSQL__(query = query, title = "Computes the average of the {}'s lower and upper outliers.".format(self.alias))
				mean_alpha, mean_1_alpha = [item[0] for item in self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].fetchall()]
	...
	by: list, optional
 	...
 	order_by: list, optional
 		List of the vcolumns used to sort the data when using TS methods.
 	print_info: bool, optional
	...
	vDataFrame[].dropna : Drops the vcolumn missing values.
		"""
		check_types([
			("method", method, ["auto", "mode", "0ifnull", 'mean', 'avg', 'median', 'ffill', 'pad', 'bfill', 'backfill'], True), 
			("by", by, [list], False), 
		...
		if ((method == "mode") and (val == None)):
			...
			if (val == None):
				print('\u26A0 Warning: The vcolumn {} has no mode (only missing values)\nNothing was filled.'.format(self.alias))
				return (self.parent)
		if (type(val) == str):
			val = val.replace("'", "''")
		if (val != None):
			...
			if (by == []):
				...
				try:
					if (fun == "MEDIAN"): fun = "APPROXIMATE_MEDIAN" 
					query = "SELECT {}, {}({}) FROM {} GROUP BY {};".format(by[0], fun, self.alias, self.parent.__genSQL__(), by[0])
					self.parent.__executeSQL__(query, title = "Computes the different aggregations.")
					result = self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].fetchall()
					...
					self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].execute("SELECT {} FROM {} LIMIT 1".format(new_column.format(self.alias), self.parent.__genSQL__()))
				except:
			...
			partition_by = "PARTITION BY {}".format(", ".join([str_column(column) for column in by])) if (by) else ""
			order_by_ts = ", ".join([str_column(column) + desc for column in order_by])
			new_column = "COALESCE({}, LAST_VALUE({} IGNORE NULLS) OVER ({} ORDER BY {}))".format("{}", "{}", partition_by, order_by_ts)
		else:
		...
		if (total > 0):
			try:
				if "count" in sauv:
					...
					self.catalog["percent"] = 100 * (int(sauv["count"]) + total) / self.parent.shape()[0]
			except:
				pass
			if (print_info):
				print("{} element(s) was/were filled".format(int(total)))
			self.parent.__add_to_history__("[Fillna]: {} missing value(s) of the vcolumn {} was/were filled.".format(total, self.alias))
		else:
			if (print_info):
	...
	vDataFrame[].mean_encode  : Encodes the vcolumn using the Mean Encoding of a response.
		...
		if (self.category() in ["date", "float"]):
			...
			self.parent.__add_to_history__("[Label Encoding]: Label Encoding was applied to the vcolumn {} using the following mapping:{}".format(self.alias, text_info))
		return (self.parent)
	#---#
	def mad(self):
		"""
	...
	def mae(self):
		"""
	---------------------------------------------------------------------------
	Aggregates the vcolumn using 'mae' (Mean Absolute Error).
	...
	vDataFrame[].get_dummies  : Encodes the vcolumn using the One Hot Encoding.
		...
		if not(self.parent[response_column].isnum()):
			...
			max_floor = len(self.parent[response_column].transformations) - len(self.transformations)
			for k in range(max_floor):
				self.transformations += [("{}", self.ctype(), self.category())]
			self.transformations += [("AVG({}) OVER (PARTITION BY {})".format(response_column,"{}"), "int", "float")]
			self.parent.__update_catalog__(erase = True, columns = [self.alias])
			self.parent.__add_to_history__("[Mean Encode]: The vcolumn {} was transformed using a mean encoding with {} as Response Column.".format(self.alias, response_column))
	...
	vDataFrame.memory_usage : Returns the vDataFrame memory usage.
		"""
		import sys
		total = (sys.getsizeof(self) + sys.getsizeof(self.alias) + sys.getsizeof(self.transformations) + sys.getsizeof(self.catalog))
		for elem in self.catalog:
	...
	vDataFrame.aggregate : Computes the vDataFrame input aggregations.
		...
		self.parent.__executeSQL__("SELECT {} FROM (SELECT {}, COUNT(*) AS _vertica_ml_python_cnt_ FROM {}{}GROUP BY {} ORDER BY _vertica_ml_python_cnt_ DESC LIMIT {}) x ORDER BY _vertica_ml_python_cnt_ ASC LIMIT 1".format(self.alias, self.alias, self.parent.__genSQL__(), where, self.alias, n))
		try:
			top = (self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].fetchone()[0])
		except:
			top = None
		if not(dropna):
			n = "" if (n == 1) else str(int(n))
			top = str(top) if (top != None) else None
			self.parent.__update_catalog__({"index": ["top{}".format(n)], self.alias: [top]})
		return (top)
	#---#
	def mul(self, 
			x: float):
	...
	vDataFrame[].nsmallest : Returns the n nsmallest vcolumn elements.
		...
		query = "SELECT * FROM {} WHERE {} IS NOT NULL ORDER BY {} DESC LIMIT {}".format(self.parent.__genSQL__(), self.alias, self.alias, n)
		query_on, time_on, title = self.parent._VERTICA_ML_PYTHON_VARIABLES_["query_on"], self.parent._VERTICA_ML_PYTHON_VARIABLES_["time_on"], "Reads {} {} largest elements.".format(self.alias, n)
		return (to_tablesample(query, self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"], name = "nlargest", query_on = query_on, time_on = time_on, title = title))
	#---#
	def normalize(self, 
				  method: str = "zscore", 
				  by: list = [], 
		...
		if (self.ctype() == "boolean"):
			...
			if (method == "zscore"):
				if (n == 0):
					...
					try:
						self.parent.__executeSQL__("SELECT {}, AVG({}), STDDEV({}) FROM {} GROUP BY {}".format(by[0], self.alias, self.alias, self.parent.__genSQL__(), by[0]), title = "Computes the different categories to normalize.")
						result = self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].fetchall()
						...
						self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].execute("SELECT {}, {} FROM {} LIMIT 1".format(avg, stddev, self.parent.__genSQL__()))
					except:
				...
				if (return_trans):
					...
					final_transformation = [("({} - {}) / {}({})".format("{}", avg, "NULLIFZERO" if (nullifzero) else "", stddev), "float", "float")]
			elif (method == "robust_zscore"):
				if (n > 0):
					print("\u26A0 Warning: the method 'robust_zscore' is available only if the parameter 'by' is empty\nIf you want to normalize by grouping by elements, please use a method in zscore|minmax")
					return(self)
				...
				if (mad != 0):
					if (return_trans):
						...
						final_transformation = [("({} - {}) / ({})".format("{}", med, mad), "float", "float")]
				else:
				...
				if (n == 0):
					...
					try:
						self.parent.__executeSQL__("SELECT {}, MIN({}), MAX({}) FROM {} GROUP BY {}".format(by[0], self.alias, self.alias, self.parent.__genSQL__(), by[0]), title = "Computes the different categories {} to normalize.".format(by[0]))
						result = self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].fetchall()
						...
						self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].execute("SELECT {}, {} FROM {} LIMIT 1".format(cmax, cmin, self.parent.__genSQL__()))
					except:
				...
				if (return_trans):
					...
					final_transformation = [("({} - {}) / {}({} - {})".format("{}", cmin, "NULLIFZERO" if (nullifzero) else "", cmax, cmin), "float", "float")]
				...
				for elem in sauv:
					if "top" in elem:
						if ("percent" in elem):
							...
							self.catalog[elem] = (sauv[elem] - sauv["50%"]) / (1.4826 * sauv["mad"])
						elif (method == "zscore"):
							self.catalog[elem] = (sauv[elem] - sauv["mean"]) / sauv["std"]
						elif (method == "minmax"):
							self.catalog[elem] = (sauv[elem] - sauv["min"]) / (sauv["max"] - sauv["min"])
			except:
				pass
			if (method == "robust_zscore"):
				...
				self.catalog["min"] = 0
				self.catalog["max"] = 1
			self.parent.__add_to_history__("[Normalize]: The vcolumn '{}' was normalized with the method '{}'.".format(self.alias,method))
		else:
	...
	vDataFrame[].nlargest : Returns the n largest vcolumn elements.
		"""
		check_types([("n", n, [int, float], False)])
		query = "SELECT * FROM {} WHERE {} IS NOT NULL ORDER BY {} ASC LIMIT {}".format(self.parent.__genSQL__(), self.alias, self.alias, n)
		query_on, time_on, title = self.parent._VERTICA_ML_PYTHON_VARIABLES_["query_on"], self.parent._VERTICA_ML_PYTHON_VARIABLES_["time_on"], "Reads {} {} smallest elements.".format(n, self.alias)
		return (to_tablesample(query, self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"], name = "nsmallest", query_on = query_on, time_on = time_on, title = title))
	#---#
	def numh(self, 
			 method: str = "auto"):
		"""
	...
	Parameters
 	...
 	float
		...
		if (self.isnum()):
			...
			min_date = self.min()
			table = "(SELECT DATEDIFF('second', '{}'::timestamp, {}) AS {} FROM {}) VERTICA_ML_PYTHON_OPTIMAL_H_TABLE".format(min_date, self.alias, self.alias, self.parent.__genSQL__())
			...
			self.parent.__executeSQL__(query, title = "Different aggregations to compute the optimal h.")
			result = self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].fetchone()
			count, vColumn_min, vColumn_025, vColumn_075, vColumn_max  = result
		else:
			raise TypeError("numh is only available on type numeric|date")
		sturges = max(float(vColumn_max - vColumn_min) / int(math.floor(math.log(count, 2) + 2)), 1e-99)
		...
		if (self.category() == "int"): best_h = max(math.floor(best_h), 1)
		return (best_h)
	#---#
	def nunique(self, 
				approx: bool = False):
	...
	Parameters
 	...
 	by: str, optional
 		vcolumn used to partition the TS.
	...
	vDataFrame.add_copy : Creates a copy of the vcolumn.
		...
		parent.__add_to_history__("[Rename]: The vcolumn {} was renamed '{}'.".format(old_name, new_name))
		return (parent)
	#---#
	def round(self, 
			  n: int):
	...
	vDataFrame.expected_store_usage : Returns the vDataFrame expected store usage.
		...
		if (pre_comp != "VERTICA_ML_PYTHON_NOT_PRECOMPUTED"):
			return (pre_comp)
		self.parent.__executeSQL__("SELECT ZEROIFNULL(SUM(LENGTH({}::varchar))) FROM {}".format(convert_special_type(self.category(), False, self.alias), self.parent.__genSQL__()), title = "Computes the Store Usage of the vcolumn {}.".format(self.alias))
		store_usage = self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].fetchone()[0]
		self.parent.__update_catalog__({"index": ["store_usage"], self.alias: [store_usage]})
		return (store_usage)
	#---#
	def str_contains(self, 
					 pat: str):
		"""
	---------------------------------------------------------------------------
	...
	vDataFrame[].head : Returns the vcolumn head.
		...
		query_on, time_on, title = self.parent._VERTICA_ML_PYTHON_VARIABLES_["query_on"], self.parent._VERTICA_ML_PYTHON_VARIABLES_["time_on"], "Reads {}.".format(self.alias)
		tail = to_tablesample("SELECT {} AS {} FROM {} LIMIT {} OFFSET {}".format(convert_special_type(self.category(), False, self.alias), self.alias, self.parent.__genSQL__(), limit, offset), self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"], query_on = query_on, time_on = time_on, title = title)
	...
	vDataFrame[].describe : Computes the vcolumn descriptive statistics.
		...
		try:
			...
			query = "SELECT SUMMARIZE_CATCOL({}::varchar USING PARAMETERS {} WITH_TOTALCOUNT = False) OVER () FROM {}".format(self.alias, topk, self.parent.__genSQL__())
			if (dropna):
				query += " WHERE {} IS NOT NULL".format(self.alias)
			self.parent.__executeSQL__(query, title = "Computes the top{} categories of {}.".format(k if k > 0 else "", self.alias))
		except:
			topk = "" if (k < 1) else "LIMIT {}".format(k)
			query = "SELECT {} AS {}, COUNT(*) AS _vertica_ml_python_cnt_, 100 * COUNT(*) / {} AS percent FROM {} GROUP BY {} ORDER BY _vertica_ml_python_cnt_ DESC {}".format(convert_special_type(self.category(), True, self.alias), self.alias, self.parent.shape()[0], self.parent.__genSQL__(), self.alias, topk)
			self.parent.__executeSQL__(query, title = "Computes the top{} categories of {}.".format(k if k > 0 else "", self.alias))
		result = self.parent._VERTICA_ML_PYTHON_VARIABLES_["cursor"].fetchall()
